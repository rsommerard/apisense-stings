...
public class WiFiDirectStingDaoGenerator {

    public static void main(String args[]) throws Exception {
        Schema schema = new Schema(1, "fr.inria.rsommerard.wifidirectsting.dao");
        schema.enableKeepSectionsByDefault();

        createDataTable(schema);
        createDevicesTable(schema);
        createMeTable(schema);

        new DaoGenerator().generateAll(schema, "app/src/main/java");
    }

    private static void createMeTable(Schema schema) {
        Entity me = schema.addEntity("Me");

        me.addIdProperty();
        me.addStringProperty("identifier").notNull();
    }
